...
parser = argparse.ArgumentParser(description='Run all testcases with golden and yours executables')
...
parser.add_argument('parameters', nargs='*', help='CMP parameters, leave blank for default')
...
for root, dirs, files in os.walk(testfolder):
    for path in sorted(dirs):
        if path != "." and path != "..":
            ...
            print("execute " + goldenRunCommand)
            chk_golden = os.system(goldenRunCommand + " > temp.txt")
            print("execute " + testRunCommand)
            chk_yours = os.system(testRunCommand + " > temp.txt")
            os.system("rm -f ./temp.txt")
            os.system("rm -f " + goldenExeName)
            os.system("rm -f " + testExeName)
            os.system("diff -b snapshot.rpt ssnapshot.rpt")
            os.system("diff -b report.rpt rreport.rpt")
